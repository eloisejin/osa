...
Data_Directory <- "/Users/eloise/Google Drive/ETSIT/Semester 1/Machine Learning Lab/Half_Term_Project_Eloise_Jin/"
...
# Data frame with only women
df_tmp3 <- replace_with_na_all(df_final,condition = ~.x == "hombre")
df_final_women <- df_tmp3 %>% drop_na()
vis_dat(df_final_women)

# Data frame with only men
df_tmp4 <- replace_with_na_all(df_final,condition = ~.x == "mujer")
df_final_men <- df_tmp4 %>% drop_na()
vis_dat(df_final_men)

# Some plots
par(mfrow=c(1, 2))
plot(df_final_men$Age, df_final_men$IAH, xlab="Age", ylab="IAH", main="AHI according the age (men)")
plot(df_final_women$Age, df_final_women$IAH, xlab="Age", ylab="IAH", main="AHI according the age (women)")